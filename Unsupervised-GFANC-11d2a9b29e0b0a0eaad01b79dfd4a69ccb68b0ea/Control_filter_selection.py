...
class Control_filter_Index_predictor():
    ...
    def predic_ID(self, noise): # predict the noise index
        ...
        construt_filter, soft_labels = Construt_filter(self.sub_filters_T.detach().numpy(), prediction.detach().cpu().numpy())
        return construt_filter, soft_labels
    ...
    def predic_ID_vector(self, primary_noise):
        ...
        for ii in range(Time_len):
            construt_filter, soft_labels = self.predic_ID(primary_noise_vectors[ii]) # construt_filter:(1, 1024)
            soft_labels = soft_labels.squeeze()
            soft_labels1 = soft_labels.tolist() # numpy to list
            soft_labels1 = [round(i, 2) for i in soft_labels1] # 保留两位小数
            print(ii+1, soft_labels1)
...
def Control_filter_selection(fs, MODEL_PTH, path_mat, Primary_noise):
